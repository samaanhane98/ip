...
BEGIN
  stream_in_s2m.tready <= (NOT reg_in_tvalid OR stream_out_s2m.tready);

  stream_out_m2s.tvalid <= reg_in_tvalid;
  stream_out_m2s.tlast <= reg_in_tlast;
  stream_out_m2s.tfirst <= reg_in_tfirst;
  stream_out_m2s.tdata <= reg_in_tdata;
  stream_out_m2s.tuser <= reg_in_tuser;

  p_input_reg : PROCESS (clk)
  BEGIN
    IF rising_edge(clk) THEN
      IF stream_in_m2s.tvalid = '1' AND (reg_in_tvalid = '0' OR stream_out_s2m.tready = '1') THEN
        reg_in_tvalid <= stream_in_m2s.tvalid;
